/**
 * @file eg_hmbt-cll_config.c
 * @brief Portable Abstracted Linked Lists Library (libpall)
 *        Hash Mod Balanced Tree CLL Config Example
 *
 * Date: 10-10-2012
 * 
...
int main(void) {
	...
	/* Configure handler with automatic search and sorted inserts.
	 *
	 * This is the same as calling:
	 * pall_hmbt_cll_set_config(hhc, CONFIG_SEARCH_AUTO |
	 *				CONFIG_INSERT_SORTED);
	 *
	 */
	hhc->set_config(hhc, CONFIG_SEARCH_AUTO | CONFIG_INSERT_SORTED);

	/* Configure handler with backward search and prev inserts
	 * hhc->set_config(hhc, CONFIG_SEARCH_BACKWARD | CONFIG_INSERT_PREV);
	 */
	/* Configure handler with forward search and next inserts
	 * hhc->set_config(hhc, CONFIG_SEARCH_FORWARD | CONFIG_INSERT_NEXT);
	 */
	 ...
	 *
	 * This is the same as calling:
	 * pall_hmbt_cll_delete(hhc, elem_val(0xdeadbeef));
	 *
	 */
