/**
 * @file eg_cll_config.c
 * @brief Portable Abstracted Linked Lists Library (libpall)
 *        Circular Linked List Config Example
 *
 * Date: 31-08-2012
 * 
...
int main(void) {
	 ...
	 * pall_cll_set_config(hc, CONFIG_SEARCH_AUTO | CONFIG_INSERT_SORTED);
	 *
	 */
	hc->set_config(hc, CONFIG_SEARCH_AUTO | CONFIG_INSERT_SORTED);

	/* Configure handler with backward search and prev inserts
	 * hc->set_config(hc, CONFIG_SEARCH_BACKWARD | CONFIG_INSERT_PREV);
	 */
	/* Configure handler with forward search and next inserts
	 * hc->set_config(hc, CONFIG_SEARCH_FORWARD | CONFIG_INSERT_NEXT);
	 */
	 ...
	 *
	 * This is the same as calling:
	 * pall_cll_delete(hc, elem_val(0xdeadbeef));
	 *
	 */
